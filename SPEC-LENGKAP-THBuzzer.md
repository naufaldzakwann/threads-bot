...
Sama seperti IGBuzzer (salah akun, lupa jadwal, pola robotic → restricted), ditambah kekhasan Threads: limit teks 500 char memaksa utas panjang, 1 topic_tag/post, flow container 2-langkah yang rawan dobel-post, dan search resmi yang dijatah mingguan sehingga riset target harus hemat.
...
4. Kampanye buzzer ke ≥ 500 akun peserta dalam 1 klik + progres real-time.
...
- Tiga engine: **Official Threads API** (utama untuk publish/reply/insight), **Unofficial Private API** (fragile, untuk follow/like/search massal), **Browser Automation threads.com** (peran LEBIH BESAR dari IG — untuk growth/reply massal & checkpoint manual).
- Growth: like, follow, unfollow, reply, repost, quote (+view timeline sebagai warm-up).
- Posting & scheduling: thread teks, image/carousel, video, reply terjadwal, quote terjadwal, repost terjadwal, topic-tag manager, AI caption utas.
- **Reply & Mention automation (PENGGANTI DM — Threads tidak punya DM mandiri via API):** unified replies-inbox, auto-reply keyword/regex/AI, sapa follower baru, mass-reply/broadcast ke utas target dengan throttling.
- **Mode Kampanye Buzzer:** like massal, reply bervariasi, repost, quote ke URL Threads target, distribusi human-like.
- Analytics: dashboard, kurva follower, log/audit, alert desktop + Telegram/Discord.
- AI LLM OpenAI-compatible untuk reply/komentar/caption utas.
...
| D3 | Engine Threads | **Tiga engine**: Official Threads API (utama publish/reply), Unofficial Private (growth/search), Browser Automation threads.com (growth/checkpoint/cadangan) — lihat §07 |
| D4 | Skala | **Ribuan akun**; limit hanya spek PC |
| D5 | Model | **Tool internal single-operator**, tanpa lisensi/server |
| D6 | AI | **LLM OpenAI-compatible** untuk reply, quote-text, caption utas; fallback template |
| D7 | Database | **SQLite (WAL)** via SQLAlchemy + Alembic; PostgreSQL-ready |
| D8 | Penyimpanan | **100% lokal**; trafik keluar hanya: graph.threads.net, threads.com/net, proxy operator, LLM operator, captcha/SMS opsional |
| D9 | Bahasa | **Indonesia** default (i18n en) |
| D10 | Rate limiting | WAJIB ada, default konservatif (tabel §19 baru, 3–5x lebih ketat dari IG); tidak bisa dimatikan |
...
| Unofficial Private | Salah SATU dipin sadar: `threads-api==1.2.0` (Danie1, arsip Okt-2023 — rapuh) atau penerus aktif (`meta-threads-sdk`/fork). SEMUA panggilan di ThreadPool, SEMUA error dinormalisasi ke E-*, naik versi hanya sadar + catat DECISIONS.md | PENGGANTI `instagrapi`; diperlakukan sebagai fragile |
| Browser | **Playwright Python + Chromium** (+`patchright`/stealth, pilih satu) `user_data_dir` per akun di `threads.com` | Peran NAIK (growth utama bila unofficial mati) |
...
Field IGBuzzer dipertahankan + adaptasi: `username` (Threads handle, unik case-insensitive), `password_enc` (password IG terhubung — untuk unofficial/browser), `email/email_password_enc + imap_*` (auto-OTP Meta), `totp_secret_enc`, `status`, `health_score`, `group_id`, `tags[]`, `priority`, `timezone` (default `Asia/Jakarta`), `locale/country`, `device_params` (fingerprint Threads: app-version + android/ios + UA — dibuat sekali, permanen), `session_blob_enc` (cookies threads.com + token unofficial), **`threads_user_id`** (ID numerik resmi), **`oauth_token_id`** (FK ke `threads_tokens`), `default_engine` (default `official_api`), `daily_limit_overrides`, `warmup_plan_id`, `proxy_id`, timestamps, soft-delete.

Tabel token baru `threads_tokens`: `id, account_id, app_id, scopes[], access_token_enc (long-lived 60 hari), refresh_token_enc?, expires_at, last_refreshed_at`. Auto-refresh H-50 via recurring job.
...
- **A. Manual:** username/handle + password IG + (opsional) email/IMAP + TOTP + grup/timezone/proxy. Opsional langsung sambung OAuth (paste auth-code atau wizard §7.6) → job `login` + `verify_oauth` (high).
- **B. Import sesi/token tanpa password:** file `session_*.json` (unofficial) atau `token.json` (OAuth). Validasi ringan (`GET /me?fields=id,username`) sebelum terapkan. Tanpa password → `needs_password`; tanpa refresh-token → `needs_reauth` saat 401.
- **C. Bulk CSV/Excel** header WAJIB: `username,password,email,email_password,proxy,totp_secret,timezone,country,group,tags,priority,threads_user_id,oauth_token` — antre login berurutan jarak 30–120 dtk, maks 20 login baru/jam global + progress WS `import.progress`.
...
- Unofficial/browser: simpan `session_blob` tiap aksi sukses (cookies berputar). Re-login auto maks 1x/akun/24 jam, global 30/jam.
...
## 7.1 `official_api` (utama publish/reply/insight/delete)
...
## 7.2 `private_unofficial` (fragile — growth/search)

- Satu library dipin sadar (default `threads-api==1.2.0` atau penerus; catat DECISIONS.md). Sinkron → ThreadPool. Login via kredensial IG + `cached_token_path` bersama untuk hemat login. Mapping error terpusat `engines/private_unofficial/errors.py` (termasuk `4279013` → `E-LIMIT-BLOCK` non-retryable).
- ✅: follow/unfollow, like/unlike, baca followers/following/likers, timeline/search luas (tanpa jatah 500/minggu), post/reply (cadangan bila official down).
- ⚠️: rapuh tiap update app Meta — WAJIB uji `scripts/check` + smoke publish tiap naik versi; bila 3x `E-ENGINE-BUG` beruntun → tandai `degraded_engine` 24 jam + fallback browser.

## 7.3 `browser` (threads.com via Playwright — peran NAIK)

- Pool terbatas + stealth + `user_data_dir` per akun + `selectors.py` + `humanize.py` (mouse/scroll/ketik) + screenshot hanya saat `E-ENGINE-BUG`.
- ✅ utama: checkpoint manual, warm-up browse, follow/like/reply/repost/quote saat unofficial mati, verifikasi visual.
- Mahal RAM (1–1.5 GB/context) — default 4 context (1–16).

## 7.4 Routing (default `official_api`)

Override job > override akun > hard rules:
- `resolve_checkpoint` / login gagal checkpoint → `browser`.
- `publish_thread/reply/quote/repost/delete/insight/keyword_search hemat` + token valid → `official_api`.
- `follow/unfollow/like/timeline_search luas` → `private_unofficial` (fallback `browser`).
...
- Chain: `official→private_unofficial→browser` untuk reply; `private_unofficial→browser→official` untuk follow/like. **Publish tidak di-fallback** (anti dobel-post; container `creation_id` sekali pakai).
...
- **Aksi:** `like, follow, unfollow` (hanya dari `followed_by_us` + opsi non-followback ≥3 hari), `reply` (1x/post/akun, hormati `reply_control`), `repost`, `quote` (wajib teks unik AI/spintax). **DIHAPUS vs IG:** `comment→reply` (rename), `view_story/save_post/poll_vote` dihapus; `view_thread/timeline` hanya untuk warm-up (tanpa kuota tulis).
- **Target:** keyword/topic search (hemat jatah resmi; default via unofficial/browser), followers/following kompetitor (cache 24 jam, cursor persisten), repliers/likers thread viral, timeline For You (browser), list manual URL/handle (txt/CSV), feed sendiri (untuk first-reply).
...
- Caption utas: spintax + variabel `{username,group_name,topic_tag:X,date,niche:X}` (maks 500, bukan 2200). **Topic-tag manager** (pengganti hashtag manager): set bernama 1-tag/post, **tanpa banned-checker** (diganti spam-word checker lokal + cek 5-link), pakai acak dari set.
...
- **Aksi:** like, **reply** (unik/akun, blok duplikat persis), **repost**, **quote** (rewrite unik), view-timeline (warm only). Preset "Amplifikasi Thread": like 100% + reply 60% + repost 30% + quote 20%. **`share_to_story/repost_to_feed/share_dm/upvote_comment` DIHAPUS**; nested discussion dipertahankan sebagai **reply-to-reply + like-reply → Top Reply**.
- **Distribusi:** `burst` (min 30 mnt/100+ akun, validator tolak <30) + `drip` truncated-exponential median spread/2 (`tasks/distribution.py`, seed deterministik; pause/resume deterministik). Tidak bypass limiter/kuota resmi.
...
- UI: daftar + wizard 4 langkah (target+preview → aksi+spintax+AI → armada+sampling → jadwal+histogram+ringkasan kuota **250/1000**) + detail per-peserta + export.
...
> Copy 90% IGBuzzer §13. Beda: batas 500 char + topic_tag + quote-rewrite.
...
- `settings, account_groups, accounts (+threads_user_id, oauth_token_id), account_tags, account_status_history, account_stats_snapshots (kolom views/likes/replies/reposts/quotes), warmup_plans, proxies, media_library, scheduled_threads (PENGGANTI scheduled_posts; kolom text_rendered, topic_tag, reply_control, link_urls[5], container_id, published_threads_id), activities, activity_cursors, interactions (action ∈ like/follow/unfollow/reply/repost/quote), campaigns, campaign_participants, campaign_schedule, thread_conversations/messages (PENGGANTI dm_*), reply_rules (PENGGANTI dm_rules), reply_campaigns/recipients (PENGGANTI dm_campaigns), jobs, job_runs, recurring_jobs, quota_usage (PK account,day TZ-akun,action + kolom bucket official: posts/replies/deletes/searches), activity_logs, alerts_history, ai_profiles/usage, personas, checkpoint_events, threads_tokens (baru), topic_sets/items (PENGGANTI hashtag_sets; 1-tag/post)`.
...
Turunan WAJIB: per-jam 20%, jitter 60–240 dtk, break tiap 15–30 aksi, night pause, IP 6/mnt, global 120/mnt. Naikkan hanya bertahap + catat DECISIONS.md + uji akun burner dulu.
...
- Startup ≤5 dtk, API p95 ≤100 ms, 1000+ akun tanpa drop, idle ≤400 MB, +≤300 MB/100 akun unofficial, 1–1.5 GB/context browser, job 100% tahan restart, log ≤500 MB.
...
- [ ] Tambah akun manual → `active` ≤60 dtk (mock).
- [ ] OAuth connect + refresh H-50 (mock 401→refresh→retry 1x).
...
- [ ] Fingerprint stabil setelah restart; sesi/token persisten.
...
- **F0 Fondasi (10%):** repo+handshake `THBUZZER_READY`+tabel inti (settings/groups/accounts/tags/history/proxies/threads_tokens/jobs/runs/logs)+settings+tray+build/check+FakeEngine → installed + "backend connected".
- **F1 Akun/Proxy/Official (20%):** login/2FA/checkpoint/fingerprint/sesi/health + **OAuth wizard + refresh + `threads_publishing_limit` guard** + router + enkripsi (termasuk token).
- **F2 Task+Growth (20%):** queue/retry/kuota/kill/recurring + limiter 3 lapis + warm-up + growth like/follow/reply via unofficial (spintax) → MVP.
- **F3 Threads Posting (15%):** library/ffmpeg/kalender/manifest/**container-flow + guard-dobel**/topic-sets/first-reply/quick-thread/repost/quote.
- **F4 Replies+AI (12%):** unified replies-inbox/rules/mass-reply + provider/persona/budget (**quote_rewrite** termasuk).
- **F5 Campaign+Analytics (13%):** distribusi deterministik + pre-flight `reply_control` + snapshot/views-likes-replies-reposts-quotes + dashboard/alert (termasuk alert kuota 250/1000 + search-mingguan).
- **F6 Unofficial-hardening+Browser+Packaging (10%):** pin unofficial + pool/stealth/humanize threads.com/captcha-generik + backup + NSIS final + QA nyata + kalibrasi §19.5 via akun burner.
...
> Pararel IGBuzzer §24 (mobile-proxy rotasi, SMS OTP, pre-flight, sleep-recovery, job-object, Telegram remote, disk-quota, single-writer) + 3 khusus Threads:

1. **Hemat search mingguan:** default semua riset target via unofficial/browser; official `keyword_search` hanya untuk verifikasi akhir + bit `search_budget_guard` (tolak aktivitas baru bila sisa <20% dengan pesan jelas).
...
3. **Relay webhook opsional (pasca-v1):** bila butuh inbox <15 mnt, sediakan relay publik kecil (VPS) yang meneruskan webhook reply/mention resmi → polling lokal tetap sebagai fallback. Tanpa relay, jangan janjikan real-time.
...
|---|---|---|---|
| thread teks/gambar/video/carousel | ✅ utama | ⚠️ cadangan | ⚠️ mahal |
| reply / first_reply | ✅ utama | ✅ | ✅ |
| quote / repost | ✅ (quote via container) | ✅ | ✅ |
| delete / hide-reply | ✅ | ❌ | ⚠️ |
| like / follow / unfollow | ❌ | ✅ utama | ✅ cadangan |
| search luas / followers / likers | ⚠️ jatah mingguan | ✅ utama | ✅ |
| insights | ✅ terbaik | ⚠️ | ❌ |
| checkpoint/manual | ❌ | ⚠️ | ✅ utama |
